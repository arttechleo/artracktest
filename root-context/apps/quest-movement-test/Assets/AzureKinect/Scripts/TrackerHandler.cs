...
{
    [Tooltip("Maximum number of bodies to visualize simultaneously. At Start the prefab's single skeleton rig (child 0) is cloned this many times so each body gets its own rig.")]
    public int maxTrackedBodies = 6;
    ...
    void Start()
    {
        // Ensure we have enough child skeleton rigs to visualize maxTrackedBodies.
        // The prefab ships with one rig at child 0; we clone it the rest of the way.
        EnsureSkeletonRigs(maxTrackedBodies);
    }

    private void EnsureSkeletonRigs(int targetCount)
    {
        if (transform.childCount == 0)
        {
            Debug.LogError("[TrackerHandler] No skeleton rig child found on this GameObject. The Kinect4AzureTracker prefab must contain at least one skeleton rig as child 0 to clone from.");
            return;
        }

        Transform template = transform.GetChild(0);
        template.gameObject.SetActive(false); // hidden by default; turned on per-frame when a body uses it
        template.name = "Skeleton_0";

        while (transform.childCount < targetCount)
        {
            Transform clone = Instantiate(template, transform);
            clone.gameObject.SetActive(false);
            clone.name = $"Skeleton_{transform.childCount - 1}";
        }
    }

    public void updateTracker(BackgroundData trackerFrameData)
    {
        // Hide all rigs first; we'll re-enable only the ones with active bodies this frame.
        for (int slot = 0; slot < transform.childCount; slot++)
        {
            transform.GetChild(slot).gameObject.SetActive(false);
        }

        int bodyCount = Mathf.Min((int)trackerFrameData.NumOfBodies, transform.childCount);
        for (int i = 0; i < bodyCount; i++)
        {
            transform.GetChild(i).gameObject.SetActive(true);
            renderSkeleton(trackerFrameData.Bodies[i], i);
        }
    }
